...
app = Flask(__name__)
app.config['SECRET_KEY'] = "Thisis deep SECRET"

class GettngCompanies(FlaskForm):
...
class generatePlot:
    ...
    def getInfo(companies):   
        ...
        mean_df.transpose()
        mean_df.plot( kind='bar',y='Compound',rot=0).figure.savefig('plot',
                    pad_inches = 10)
...
def hello_world():
    form = GettngCompanies()
    f = generatePlot.getInfo('tcs')
    # if form.is_submitted():

        # a = form.names.data
        # print(a)
        # getInfo(a)
    return render_template('getform.html',form=form)
